...
function addFooter(slide, pageNum) {
  slide.addText(
    [
      ...
      { text: "Carbon Decision Terminal", options: { color: C.soft, fontSize: 10, italic: true, fontFace: FONT_BODY } },
    ],
    { x: 0.5, y: H - 0.45, w: 6, h: 0.3, margin: 0, valign: "middle" }
  );
  slide.addText(`${pageNum} / ${TOTAL}`, {
    x: W - 1.2, y: H - 0.45, w: 0.7, h: 0.3,
    color: C.mute, fontSize: 10, align: "right", valign: "middle",
    fontFace: FONT_MONO, margin: 0,
  });
}

function addTitleHead(slide, ko, en) {
  slide.addText(ko, {
    x: 0.55, y: 0.5, w: W - 1.1, h: 0.7,
    fontSize: 30, fontFace: FONT_HEAD, color: C.ink, bold: true,
    align: "left", valign: "middle", margin: 0,
  });
  if (en) {
    slide.addText(en, {
      x: 0.55, y: 1.18, w: W - 1.1, h: 0.36,
      fontSize: 14, fontFace: FONT_BODY, color: C.soft, italic: true,
      align: "left", valign: "middle", margin: 0,
    });
  }
}
...
// ── Slide 1: Title (dark hero) ────────────────────────────────────────
{
  ...
  s.addText("CARBON DECISION TERMINAL · v1.3", {
    x: 0.9, y: 1.2, w: 9, h: 0.5,
    fontSize: 14, fontFace: FONT_BODY, color: "5A99FF",
    bold: true, charSpacing: 6, margin: 0,
  });
  s.addText("C-Quant", {
    x: 0.9, y: 1.8, w: 9, h: 1.6,
    fontSize: 96, fontFace: FONT_HEAD, color: "FFFFFF",
    bold: true, margin: 0,
  });
  s.addText("탄소배출권 의사결정 터미널", {
    x: 0.9, y: 3.5, w: 11, h: 0.7,
    fontSize: 30, fontFace: FONT_HEAD, color: "FFFFFF", margin: 0,
  });
  s.addText("EU ETS · K-ETS · China ETS  —  하나의 데스크에서", {
    x: 0.9, y: 4.3, w: 11, h: 0.5,
    fontSize: 18, fontFace: FONT_BODY, color: "CADCFC", italic: true,
    margin: 0,
  });
  s.addShape(pres.shapes.LINE, {
    x: 0.9, y: H - 1.3, w: 4, h: 0,
    line: { color: C.accent, width: 2 },
  });
  s.addText("Research · Monitoring · Forecasting · Briefing", {
    x: 0.9, y: H - 1.15, w: 11, h: 0.4,
    fontSize: 12, fontFace: FONT_BODY, color: "8B95A1",
...
// ── Slide 2: The decision ─────────────────────────────────────────────
{
  ...
  s.addText(
    ...
    {
      x: 3.0, y: 2.2, w: 9.8, h: 3.0,
      fontSize: 50, fontFace: FONT_HEAD, bold: true, valign: "middle",
  ...
  s.addText(
    ...
    {
      x: 1.5, y: 5.6, w: 10.8, h: 0.9,
      fontSize: 16, fontFace: FONT_BODY, color: C.body,
...
// ── Slide 3: The problem ──────────────────────────────────────────────
{
  ...
  cards.forEach((card, i) => {
    ...
    s.addShape(pres.shapes.RECTANGLE, {
      x: cx, y: startY, w: 0.08, h: colH,
      fill: { color: C.accent }, line: { color: C.accent, width: 0 },
    });
    // Icon at top-right of card
    s.addImage({ data: card.icon, x: cx + colW - 0.85, y: startY + 0.3, w: 0.55, h: 0.55 });
    s.addText(card.n, {
      x: cx + 0.3, y: startY + 0.25, w: 1.5, h: 0.45,
      fontSize: 14, fontFace: FONT_MONO, color: C.accent,
      bold: true, charSpacing: 4, margin: 0,
    });
    s.addText(card.title, {
      x: cx + 0.3, y: startY + 0.95, w: colW - 0.6, h: 0.7,
      fontSize: 22, fontFace: FONT_HEAD, color: C.ink, bold: true, margin: 0,
    });
    s.addText(card.body, {
      x: cx + 0.3, y: startY + 1.75, w: colW - 0.6, h: colH - 1.9,
...
// ── Slide 4: Product boundary ─────────────────────────────────────────
{
  ...
  s.addText("DOES", {
    x: startX + 0.4, y: startY + 0.35, w: colW - 1.4, h: 0.4,
    fontSize: 13, fontFace: FONT_BODY, color: C.accentInk,
    bold: true, charSpacing: 6, margin: 0,
  });
  s.addText("연구 · 모니터링 · 예측 · 브리핑", {
    x: startX + 0.4, y: startY + 0.85, w: colW - 0.8, h: 0.7,
    fontSize: 22, fontFace: FONT_HEAD, color: C.ink, bold: true, margin: 0,
  });
  s.addText(
    [
      { text: "공식 anchor 가격, freshness, 접근 방식을 한 화면에", options: { bullet: true, breakLine: true } },
      { text: "정책-가격 catalyst 21개를 시나리오로 정리", options: { bullet: true, breakLine: true } },
      { text: "리얼타임 active pattern 자동 감지", options: { bullet: true, breakLine: true } },
      { text: "buy / hold / reduce posture + 신뢰도 + 근거 제시", options: { bullet: true, breakLine: true } },
      { text: "외부 감사를 위한 calibration provenance 표기", options: { bullet: true } },
    ],
    {
      x: startX + 0.4, y: startY + 1.7, w: colW - 0.8, h: colH - 1.9,
  ...
  s.addShape(pres.shapes.RECTANGLE, {
    ...
    fill: { color: "FDF1F2" }, line: { color: C.red, width: 0 },
  });
  s.addImage({ data: icons.doNo, x: rightX + colW - 0.95, y: startY + 0.35, w: 0.55, h: 0.55 });
  s.addText("DOES NOT", {
    x: rightX + 0.4, y: startY + 0.35, w: colW - 1.4, h: 0.4,
    fontSize: 13, fontFace: FONT_BODY, color: C.red,
    bold: true, charSpacing: 6, margin: 0,
  });
  s.addText("주문 · 보관 · 정산 · 브로커리지", {
    x: rightX + 0.4, y: startY + 0.85, w: colW - 0.8, h: 0.7,
    fontSize: 22, fontFace: FONT_HEAD, color: C.ink, bold: true, margin: 0,
  });
  s.addText(
    [
      { text: "주문 라우팅 / 거래 체결 — 라이선스 브로커 사용", options: { bullet: true, breakLine: true } },
      { text: "자산 보관 / 정산 — 등록기관 / 수탁자 사용", options: { bullet: true, breakLine: true } },
      { text: "1:1 개인 맞춤 매수/매도 지시", options: { bullet: true, breakLine: true } },
      { text: "공식 공시 문서를 대체하지 않음", options: { bullet: true, breakLine: true } },
      { text: "기관 가격을 fabricate 하지 않음 — not-configured 상태로 노출", options: { bullet: true } },
    ],
    {
      x: rightX + 0.4, y: startY + 1.7, w: colW - 0.8, h: colH - 1.9,
      fontSize: 14, fontFace: FONT_BODY, color: C.ink,
      paraSpaceAfter: 8, margin: 0,
    }
  );
...
// ── Slide 5: Signal stack (8 layers) — with icons ─────────────────────
{
  ...
  const layers = [
    ...
    [icons.matrix,   "Layer 2", "Driver matrix",          "약 47개 드라이버 × 6 패밀리 (정책, 전력, 연료, 매크로, 날씨, 미시구조)"],
    [icons.catalyst, "Layer 3", "Catalyst combinations",  "≥ 2 driver를 묶은 21개 시나리오 (cold-snap stack, MSR + Fit-for-55, ETS2 등)"],
    [icons.active,   "Layer 4", "Active patterns",        "freshness / 가격급변 / 거래량급변 / 프록시 괴리 — 임계 자동감지"],
    [icons.calib,    "Layer 5", "Calibration",            "25개 historical event 기반 event study — heuristic / backtest / calibrated"],
    [icons.proxy,    "Layer 6", "Listed proxy",           "ICE EUA · KRBN · KEUA · CO2.L · KCCA — 프록시 괴리 시그널"],
    [icons.atlas,    "Layer 7", "Materials atlas",        "IPCC AR6 · IEA · IRENA · ICVCM 인용 장기 abatement curve"],
    [icons.feeds,    "Layer 8", "Public-data feeds",      "FRED · ECB SDW · ICAP · World Bank — license-free 보강"],
  ];

  const startY = 1.85;
  const rowH = 0.55;
  const colIconW = 0.5, colNumW = 0.95, colTitleW = 2.6;
  ...
  layers.forEach((row, i) => {
    ...
    if (i % 2 === 1) {
      s.addShape(pres.shapes.RECTANGLE, {
        x: 0.5, y: y, w: W - 1.0, h: rowH,
        fill: { color: C.panel }, line: { color: C.panel, width: 0 },
      });
    }
    s.addImage({ data: row[0], x: 0.65, y: y + 0.1, w: 0.35, h: 0.35 });
    s.addText(row[1], {
      x: 0.65 + colIconW + 0.1, y: y, w: colNumW, h: rowH,
      fontSize: 12, fontFace: FONT_MONO, color: C.accent,
      bold: true, valign: "middle", margin: 0,
    });
    s.addText(row[2], {
      x: 0.65 + colIconW + 0.1 + colNumW, y: y, w: colTitleW, h: rowH,
      fontSize: 16, fontFace: FONT_HEAD, color: C.ink,
      bold: true, valign: "middle", margin: 0,
    });
    s.addText(row[3], {
      x: 0.65 + colIconW + 0.1 + colNumW + colTitleW, y: y, w: colDescW, h: rowH,
...
// ── Slide 6: Decision surfaces ────────────────────────────────────────
{
  ...
  const surfaces = [
    { name: "Command", q: "오늘 무엇을 해야 하고, 왜 그런가?", desc: "공식 anchor + proxy 차트 + 결정 메모 + 상위 드라이버 + freshness chips를 한 화면에." },
    { name: "Drivers", q: "지금 어떤 시그널이 켜졌는가?",       desc: "Active patterns / 21 catalyst / materials atlas / calibration provenance / event timeline / driver heatmap." },
    { name: "Desk",    q: "한 시장을 깊게 보고 싶다",          desc: "anchor vs hedge tape 차트, range/correlation 표, scenario weight 슬라이더 — 시장별 brief 작성용." },
    { name: "Sources", q: "이 데이터는 어디서 왔고 얼마나 신선한가?", desc: "접근 방식, freshness, 입력 커버리지, 신뢰 레지스트리 — 컴플라이언스 리뷰의 첫 화면." },
  ];

  const colW = 2.95, colH = 4.0, gap = 0.2;
  ...
  surfaces.forEach((sf, i) => {
    ...
    s.addShape(pres.shapes.RECTANGLE, {
      x: cx, y: startY, w: colW, h: 0.6,
      fill: { color: C.accent }, line: { color: C.accent, width: 0 },
    });
    s.addText(sf.name, {
      x: cx + 0.2, y: startY + 0.05, w: colW - 0.4, h: 0.5,
      fontSize: 18, fontFace: FONT_HEAD, color: "FFFFFF",
      bold: true, valign: "middle", margin: 0,
    });
    s.addText("“" + sf.q + "”", {
      x: cx + 0.2, y: startY + 0.85, w: colW - 0.4, h: 1.4,
      fontSize: 13, fontFace: FONT_BODY, color: C.ink, italic: true,
      valign: "top", margin: 0, paraSpaceAfter: 4,
    });
    s.addText(sf.desc, {
      x: cx + 0.2, y: startY + 2.3, w: colW - 0.4, h: colH - 2.4,
      fontSize: 12, fontFace: FONT_BODY, color: C.body,
      valign: "top", margin: 0, paraSpaceAfter: 4,
    });
  });
...
// ── Slide 7: Calibration provenance — with chart ─────────────────────
{
  ...
  const states = [
    { tag: "heuristic",  icon: icons.heuristic, color: C.yellow, count: 21,
      label: "휴리스틱 상수", desc: "interactionEffect별 placeholder 상수. event log로 검증되지 않음. v1.3 기본 상태." },
    { tag: "backtest",   icon: icons.backtest,  color: C.accent, count: 0,
      label: "이벤트 스터디 결과", desc: "≥ 2 events · walk-forward 기반 multiplier. 25개 historical event log + monthly anchors 사용." },
    { tag: "calibrated", icon: icons.calibrated, color: C.green, count: 0,
      label: "백테스트 + 모델 owner 검토", desc: "backtest 후 model owner 사인오프. CHANGELOG에 multiplier · hit rate · 날짜 기록." },
  ];
  ...
  states.forEach((st, i) => {
    ...
    s.addShape(pres.shapes.RECTANGLE, {
      x: badgeX, y, w: 6.4, h: badgeRowH,
      fill: { color: C.bg }, line: { color: st.color, width: 1.5 },
    });
    s.addImage({ data: st.icon, x: badgeX + 0.25, y: y + 0.35, w: 0.8, h: 0.8 });
    s.addText(st.tag, {
      x: badgeX + 1.2, y: y + 0.18, w: 4.5, h: 0.35,
      fontSize: 13, fontFace: FONT_MONO, color: st.color,
      bold: true, charSpacing: 4, margin: 0,
    });
    s.addText(st.label, {
      x: badgeX + 1.2, y: y + 0.48, w: 4.5, h: 0.4,
      fontSize: 16, fontFace: FONT_HEAD, color: C.ink, bold: true, margin: 0,
    });
    s.addText(st.desc, {
      x: badgeX + 1.2, y: y + 0.88, w: 5.0, h: badgeRowH - 0.95,
      fontSize: 11, fontFace: FONT_BODY, color: C.body, margin: 0,
    });
  });

  // Right side: a real bar chart showing distribution
  s.addText("21개 시나리오 현재 상태", {
    x: 7.4, y: 2.0, w: 5.4, h: 0.4,
    fontSize: 14, fontFace: FONT_HEAD, color: C.ink, bold: true, margin: 0,
  });
  s.addText("(2026-05-06 기준)", {
    x: 7.4, y: 2.4, w: 5.4, h: 0.3,
    fontSize: 11, fontFace: FONT_BODY, color: C.soft, italic: true, margin: 0,
  });
  ...
  s.addText(
    "v1.3 기준 21개 시나리오 모두 heuristic. 외부 검토자가 이벤트 로그 + price anchors로 backtest 단계 진입 가능. npm run calibration:check가 90일 freshness 게이트 강제.",
    {
      ...
      fontSize: 11, fontFace: FONT_BODY, color: C.soft, italic: true,
      valign: "top", margin: 0,
    }
  );
...
// ── Slide 8: Compliance per jurisdiction ──────────────────────────────
{
  ...
  const rows = [
    { flag: "EU", title: "European Union", body: "MiFID II 투자서비스 / 자기매매 / 주문전달 X. EU Benchmarks Regulation 의 published index X. EEX, EU Commission, ICAP 1차 자료 인용." },
    { flag: "KR", title: "Korea (KRX K-ETS)", body: "자본시장법 투자자문업·일임업 X. 금융투자업자 X. 배출권 거래소·검증기관 X. KRX, MOE, ICAP 1차 자료 인용." },
    { flag: "CN", title: "China (national ETS)", body: "증권법 증권사 / 자산관리 X. 등록 탄소거래플랫폼 X — SHEEX가 운영주체. CSRC 라이선스 자문 X. MEE, SHEEX, ICAP 1차 자료 인용." },
  ];
  ...
  rows.forEach((r, i) => {
    ...
    s.addShape(pres.shapes.RECTANGLE, {
      x: 0.6, y: y, w: W - 1.2, h: rowH,
      fill: { color: C.panel }, line: { color: C.line, width: 0.75 },
    });
    s.addShape(pres.shapes.RECTANGLE, {
      x: 0.6, y: y, w: 1.2, h: rowH,
      fill: { color: C.ink }, line: { color: C.ink, width: 0 },
    });
    s.addText(r.flag, {
      x: 0.6, y: y, w: 1.2, h: rowH,
      fontSize: 36, fontFace: FONT_HEAD, color: "FFFFFF",
      bold: true, align: "center", valign: "middle", margin: 0,
    });
    s.addText(r.title, {
      x: 2.0, y: y + 0.15, w: W - 2.7, h: 0.5,
      fontSize: 18, fontFace: FONT_HEAD, color: C.ink, bold: true, margin: 0,
    });
    s.addText(r.body, {
      x: 2.0, y: y + 0.65, w: W - 2.7, h: rowH - 0.7,
...
// ── Slide 9: Tech ─────────────────────────────────────────────────────
{
  ...
  const tiles = [
    ...
    { k: "Packaging",  v: "electron-builder",        ext: "Windows portable + NSIS · macOS / Linux advisory" },
    { k: "Auto-update",v: "electron-updater",        ext: "GitHub releases 피드 · code-signed 권장" },
    { k: "Charts",     v: "Lightweight Charts",      ext: "Toss-style 디자인 토큰 · Pretendard / Inter / Fraunces" },
    { k: "Telemetry",  v: "Sentry + analytics",      ext: "둘 다 opt-in · 환경변수 게이트 · 기본 OFF" },
  ];

  const colW = 4.0, colH = 1.8, gap = 0.3;
  const startX = (W - (colW * 3 + gap * 2)) / 2;
  const startY = 2.0;
  ...
  tiles.forEach((t, i) => {
    ...
    s.addShape(pres.shapes.RECTANGLE, {
      x: cx, y: cy, w: colW, h: colH,
      fill: { color: C.bg }, line: { color: C.line, width: 1 },
    });
    s.addShape(pres.shapes.RECTANGLE, {
      x: cx, y: cy, w: 0.08, h: colH,
      fill: { color: C.accent }, line: { color: C.accent, width: 0 },
    });
    s.addText(t.k, {
      x: cx + 0.3, y: cy + 0.2, w: colW - 0.5, h: 0.35,
      fontSize: 11, fontFace: FONT_BODY, color: C.soft,
      charSpacing: 4, bold: true, margin: 0,
    });
    s.addText(t.v, {
      x: cx + 0.3, y: cy + 0.55, w: colW - 0.5, h: 0.55,
      fontSize: 18, fontFace: FONT_HEAD, color: C.ink, bold: true, margin: 0,
    });
    s.addText(t.ext, {
      x: cx + 0.3, y: cy + 1.15, w: colW - 0.5, h: colH - 1.2,
...
// ── Slide 10: Architecture — diagram with arrows ──────────────────────
{
  ...
  const boxes = [
    { icon: icons.rendererIcn, k: "Renderer", v: "src/**", body: "React UI · 4 surfaces · 시장 보드 · 드라이버 · 소스" },
    { icon: icons.bridgeIcn,   k: "Preload",  v: "preload.js", body: "contextBridge로 typed allow-listed API 만 노출" },
    { icon: icons.serverIcn,   k: "Main",     v: "main.js · electron/*.js", body: "라이프사이클 · IPC · 공식 fetch · 영속화 · CSP" },
  ];

  const boxW = 3.4, boxH = 2.6;
  const gap = 0.6;
  ...
  boxes.forEach((b, i) => {
    ...
    s.addShape(pres.shapes.RECTANGLE, {
      x: cx, y: startY, w: boxW, h: boxH,
      fill: { color: C.panel }, line: { color: C.line, width: 0.75 },
    });
    s.addImage({ data: b.icon, x: cx + (boxW - 0.7) / 2, y: startY + 0.3, w: 0.7, h: 0.7 });
    s.addText(b.k, {
      x: cx, y: startY + 1.1, w: boxW, h: 0.4,
      fontSize: 18, fontFace: FONT_HEAD, color: C.accent, bold: true,
      align: "center", margin: 0,
    });
    s.addText(b.v, {
      x: cx, y: startY + 1.55, w: boxW, h: 0.3,
      fontSize: 11, fontFace: FONT_MONO, color: C.body,
      align: "center", margin: 0,
    });
    s.addText(b.body, {
      x: cx + 0.2, y: startY + 1.95, w: boxW - 0.4, h: boxH - 2.05,
    ...
    if (i < boxes.length - 1) {
      ...
      s.addShape(pres.shapes.LINE, {
        x: arrowX0, y: arrowY, w: arrowX1 - arrowX0, h: 0,
        line: { color: C.accent, width: 2.5, endArrowType: "triangle" },
      });
      // label between arrows
      s.addText(i === 0 ? "IPC" : "events", {
        x: arrowX0, y: arrowY - 0.35, w: arrowX1 - arrowX0, h: 0.25,
        fontSize: 9, fontFace: FONT_MONO, color: C.soft,
  ...
  s.addShape(pres.shapes.RECTANGLE, {
    x: 0.6, y: baseY, w: W - 1.2, h: 1.0,
    fill: { color: C.accentSoft }, line: { color: C.accent, width: 0 },
  });
  s.addText("SECURITY BASELINE", {
    x: 0.8, y: baseY + 0.15, w: W - 1.6, h: 0.3,
    fontSize: 11, fontFace: FONT_BODY, color: C.accentInk,
    bold: true, charSpacing: 4, margin: 0,
  });
  s.addText(
    "contextIsolation true · nodeIntegration false · sandbox true · webSecurity true · strict CSP · assertTrustedSender on every IPC",
    {
      x: 0.8, y: baseY + 0.45, w: W - 1.6, h: 0.4,
      fontSize: 12, fontFace: FONT_BODY, color: C.ink, margin: 0,
    }
  );

  s.addText(
    "영속화는 모두 <userData> 아래 — settings · watchlist · backtests · logs.",
    {
      x: 0.6, y: H - 0.95, w: W - 1.2, h: 0.4,
      fontSize: 11, fontFace: FONT_BODY, color: C.soft,
...
// ── Slide 11: Roadmap — with icons ────────────────────────────────────
{
  ...
  const items = [
    { icon: icons.spread,    tag: "Spread",    title: "Clean dark · clean spark spread monitor", body: "검증된 fuel · power 입력으로 fuel-switching 마진을 실시간으로." },
    { icon: icons.errBand,   tag: "Backtest",  title: "Historical forecast error panel + rolling confidence", body: "OOS forecast 검증을 walk-forward harness로 자체 노출." },
    { icon: icons.portfolio, tag: "Portfolio", title: "Carbon sleeve optimizer", body: "다중 시장 보유 비중 최적화 + 비용 가정 명시." },
    { icon: icons.exportIcn, tag: "Export",    title: "Read-only institutional export layer", body: "데스크 통합용 read-only API · CSV / JSON snapshot 게이트." },
  ];

  const startY = 2.0, rowH = 1.0;
...
// ── Slide 12: Closing ─────────────────────────────────────────────────
{
  ...
  s.addText("매일 같은 4단계", {
    x: 0.9, y: 1.5, w: 11, h: 0.5,
    fontSize: 13, fontFace: FONT_BODY, color: "5A99FF",
    bold: true, charSpacing: 8, margin: 0,
  });

  const steps = ["공식", "프록시", "드라이버", "결정"];
  const stepW = 2.4, stepGap = 0.3;
  const totalW = steps.length * stepW + (steps.length - 1) * stepGap;
  const startX = (W - totalW) / 2;
  steps.forEach((step, i) => {
    const x = startX + i * (stepW + stepGap);
    s.addShape(pres.shapes.RECTANGLE, {
      x, y: 2.4, w: stepW, h: 1.5,
      fill: { color: "232830" }, line: { color: "323840", width: 1 },
    });
    s.addText(`0${i + 1}`, {
      x, y: 2.5, w: stepW, h: 0.4,
      fontSize: 12, fontFace: FONT_MONO, color: C.accent,
      bold: true, align: "center", valign: "middle", margin: 0,
    });
    s.addText(step, {
      x, y: 2.95, w: stepW, h: 0.9,
      fontSize: 32, fontFace: FONT_HEAD, color: "FFFFFF", bold: true,
      align: "center", valign: "middle", margin: 0,
    });
    if (i < steps.length - 1) {
      s.addText("→", {
        x: x + stepW, y: 2.4, w: stepGap, h: 1.5,
        fontSize: 22, fontFace: FONT_BODY, color: C.accent, bold: true,
        align: "center", valign: "middle", margin: 0,
      });
    }
  });
  ...
  s.addText("C-Quant이 도와드립니다.", {
    x: 0.9, y: 4.6, w: 11, h: 0.7,
    fontSize: 28, fontFace: FONT_HEAD, color: "FFFFFF", bold: true, margin: 0,
  });
  s.addText("Carbon decisions, every day, in the same shape.", {
    x: 0.9, y: 5.3, w: 11, h: 0.5,
    fontSize: 14, fontFace: FONT_BODY, color: "8B95A1", italic: true, margin: 0,
  });

  s.addShape(pres.shapes.LINE, {
    x: 0.9, y: H - 1.2, w: 4, h: 0,
    line: { color: C.accent, width: 2 },
  });
  s.addText(
    "github.com/hyunjin-kor/C-Quant  ·  research-only  ·  v1.3",
    {
      x: 0.9, y: H - 1.05, w: 11, h: 0.4,
      fontSize: 11, fontFace: FONT_MONO, color: "8B95A1",
