...
test("quote label helpers localize known quote titles in Korean", () => {
  ...
  assert.equal(
    localization.localizeQuoteTitleLabel(
      "ko",
      "eua-dec-benchmark",
  ...
  assert.equal(
    localization.localizeQuoteTitleLabel("ko", "ttf-gas-future", "Dutch TTF gas future"),
    "네덜란드 TTF 가스 선물"
  );
  assert.equal(
    localization.localizeQuoteTitleLabel("ko", "brent-future", "Brent crude future"),
    "브렌트 원유 선물"
  );
  assert.equal(
    localization.localizeQuoteTitleLabel("ko", "krbn-proxy", "KRBN global carbon ETF"),
    "KRBN 글로벌 탄소 ETF"
  );
});
...
test("quote label helpers fall back safely for unknown inputs", () => {
  assert.equal(
    localization.localizeQuoteTitleLabel("ko", "custom-quote", "Custom structured product"),
    "Custom structured product"
  );
  assert.equal(
    localization.localizeQuoteCategoryLabel("ko", "Cross-market sleeve"),
    "Cross-market sleeve"
  );
