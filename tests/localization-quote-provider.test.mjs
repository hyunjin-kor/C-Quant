...
test("quote provider helper localizes known provider labels in Korean", () => {
  ...
  assert.equal(
    localization.localizeQuoteProviderLabel("ko", "Yahoo Finance web chart feed"),
    "Yahoo Finance 웹 차트 소스"
  );
  assert.equal(
    localization.localizeQuoteProviderLabel("ko", "Official web flow"),
    "공식 웹 플로우"
  );
...
test("quote provider helper handles regex-style provider variants in Korean", () => {
  assert.equal(
    localization.localizeQuoteProviderLabel("ko", "Yahoo Finance delayed web chart feed"),
    "Yahoo Finance 웹 차트 소스"
  );
  assert.equal(
    localization.localizeQuoteProviderLabel("ko", "Internal web chart feed"),
    "참고용 웹 차트 소스"
  );
});
...
test("quote provider helper keeps English provider labels unchanged", () => {
  assert.equal(
    localization.localizeQuoteProviderLabel("en", "Yahoo Finance web chart feed"),
    "Yahoo Finance web chart feed"
  );
...
test("quote provider helper returns stable fallbacks for empty and unknown values", () => {
  ...
  assert.equal(
    localization.localizeQuoteProviderLabel("en", "Internal vendor relay"),
