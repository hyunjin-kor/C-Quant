...
- [ ] Finish KO / EN parity for remaining provider, source-note, benchmark, driver, official-source, and data-driven strings; older long-form detail strings still need a visual pass. *(Code-level audit clean as of 2026-05-06: all `ko: "..."` entries in [src/lib/localization.ts](../src/lib/localization.ts) have a paired `en: "..."` (Node-script asymmetry check). 53/53 regression tests passing across 15 helper slices. Remaining work is purely a runtime visual pass on long-form detail strings — wording quality, not missing translations.)*
...
- [ ] Create canonical Figma screens for the four primary surfaces. *(External deliverable — not advanceable from a code-only autonomous cycle. Reference implementation lives inline in [src/App.tsx](../src/App.tsx) (`renderCommand` / `renderDesk` / `renderDrivers` / `renderSources`); the blocker is producing the Figma artefacts and committing their links to [docs/project-links.md](project-links.md).)*
...
- All four surface render paths (`Command`, `Desk`, `Drivers`, `Sources`) plus the `Inspector` and shared preparation logic remain inline in `src/App.tsx`. An earlier attempt to extract `Command` / `Desk` / `Sources` into stand-alone components created staged modules under `src/components/surfaces/` but they were never wired into the renderer; those files were removed as dead code. App.tsx remains larger and riskier than it should be.
...
The work they described — V1 surface extraction (`Command` / `Desk` / `Sources`) and the KO/EN parity sweeps — is reflected in the current source under [src/App.tsx](../src/App.tsx) (`renderCommand` / `renderDesk` / `renderDrivers` / `renderSources` inline) and [src/lib/localization.ts](../src/lib/localization.ts). A staged extraction of three surface modules under `src/components/surfaces/` was never wired up and has been removed; the surface implementations remain inline pending a future refactor. Use the current sources as the live reference instead of the missing planning files. Future planning artifacts should be committed under `docs/` so the autonomy ledger stays self-contained.
